...
OUTPUT_DIR = "/content/drive/MyDrive/diplomka/debug_audio_griffinlimFinal"

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
TRUNCATION_PSI = 0.8


def denormalize_to_linear_amplitude(s_norm):
    """
    Reverses the normalization process to retrieve linear amplitude required by Griffin-Lim.
    """
...
def save_spectrogram(tensor, output_path, title):
    """
    Converts linear amplitude back to decibels for readability and saves the plot as an image.
    """
    spec_np = tensor.squeeze().cpu().numpy()
    spec_db_visual = librosa.amplitude_to_db(spec_np, ref=np.max)

    plt.figure(figsize=(10, 4))
    librosa.display.specshow(spec_db_visual, sr=SAMPLE_RATE, hop_length=HOP_LENGTH, x_axis='time', y_axis='mel', cmap='magma')
...
def run_griffinlim_nice_inference():
    """
    Executes the full generation pipeline to synthesize audio from text prompts using the Griffin-Lim algorithm.
    """
    ...
    print("Initializing scaling and phase estimation tools...")
    ...
    with torch.no_grad():
        for i, prompt in enumerate(test_prompts):
            ...
            s_amp = denormalize_to_linear_amplitude(dithered_spec.squeeze(1).float())

            if s_amp.dim() == 4:
                s_amp = s_amp.squeeze(1)

            spec_output_path = os.path.join(OUTPUT_DIR, f"test_sample_{i+1}_{prompt.replace(' ', '_')}_griffinlim_spec.png")
            save_spectrogram(s_amp, spec_output_path, f"Griffin-Lim Input (Linear Amplitude): {prompt}")

            with torch.amp.autocast('cuda'):
                linear_spec = inverse_mel(s_amp)
                audio_waveform = griffin_lim(linear_spec).cpu()
            ...
            output_path = os.path.join(OUTPUT_DIR, f"test_sample_{i+1}_{prompt.replace(' ', '_')}_griffinlim_nice.wav")
